...
// Global click handler shared via event delegation — only one needed per card
...
const openLightbox = (e) => {
  ...
  if (typeof window !== 'undefined' && window.openLightbox) {
    window.openLightbox(props.photo.id);
  }
};

const toggleFlip = (e) => {
  if (e.target.tagName === 'INPUT' || e.target.tagName === 'BUTTON' || e.target.closest('button')) return;
  if (e.target.closest('.img-container')) return; // image click opens lightbox instead
  vibrate(8);
  ...
  <div ref="wrapperRef" class="card-wrapper" :class="{ 'is-visible': isVisible }" :style="`--random-angle: ${randomAngle}deg;`">
    <div class="card-inner" :class="{ 'is-flipped': isFlipped }" @click="toggleFlip">
      ...
      <div class="card-front">
        <div class="img-container" :style="{ aspectRatio: safeAspectRatio }" @click="openLightbox">
          <img 
            :src="photo.image_url" 
...
.card-wrapper {
  ...
  opacity: 0;
  transition: opacity var(--duration-slow) ease, transform 0.8s var(--ease-out-back);
...
.card-inner {
  ...
  transition: transform 0.55s var(--ease-out-back), box-shadow var(--duration-normal) ease;
  cursor: pointer; border-radius: 18px; will-change: transform;
}
.card-inner.is-flipped { transform: rotateY(180deg); }

@media (hover: hover) {
  .card-wrapper:hover .card-inner:not(.is-flipped) {
    transform: translate3d(0, -6px, 10px) scale(1.02) rotate(0.2deg);
    box-shadow: var(--shadow-glow);
  }
}

.card-front, .card-back {
  position: relative; width: 100%; backface-visibility: hidden; border-radius: 18px;
...
.img-container img { width: 100%; height: 100%; object-fit: contain; display: block; opacity: 0; filter: blur(12px); transform: scale(1.04); transition: opacity var(--duration-slow) ease, filter 0.6s var(--ease-out-expo), transform 0.6s var(--ease-out-expo); }
.img-container img.loaded { opacity: 1; filter: blur(0); transform: scale(1); }

:deep(.highlight-text) { color: var(--accent-color); font-weight: 800; background: rgba(140, 161, 146, 0.15); border-radius: 4px; padding: 0 4px; box-shadow: 0 0 8px rgba(140, 161, 146, 0.2); transition: all var(--duration-normal) ease; }
...
.like-btn { position: absolute; right: 0; bottom: -2px; background: transparent; border: none; display: flex; align-items: center; gap: 6px; color: var(--accent-color); font-size: 13px; font-weight: 700; cursor: pointer; transition: var(--duration-normal); }
...
.comment-item { margin-bottom: 16px; line-height: 1.6; animation: fadeInSmooth var(--duration-slow) ease backwards; }
...
.comment-input-box input { flex: 1; padding: 12px 15px; border: 1px solid var(--border-light); border-radius: 14px; font-size: 13px; outline: none; background: var(--bg-base); transition: var(--duration-normal); }
.comment-input-box input:focus { border-color: var(--accent-color); background: #fff; box-shadow: 0 2px 10px rgba(140,161,146,0.1); }
.comment-input-box button { background: var(--accent-color); color: white; border: none; border-radius: 12px; padding: 0 18px; font-size: 13px; font-weight: bold; cursor: pointer; transition: var(--duration-normal); }
